...
const targetUrl = String(
  positionals[0] || values.url || "https://yitang.top/",
).trim();
...
try {
  ...
  const terminalQr =
    ...
    (await renderTerminalQrFromPng(screenshotPath).catch(() => ""));
...
async function waitForQrReady(page) {
  ...
  for (;;) {
    const mainImgReady = await page
      .evaluate(() => {
        const img = document.querySelector(
          "img.qrcode.lightBorder.js_qrcode_img",
        );
        return Boolean(
    ...
    if (iframeEl) {
      ...
      if (box && box.width > 120 && box.height > 120)
        return { kind: "login-iframe" };
    }

    if (Date.now() - startedAt > QR_SCAN_TIMEOUT_MS) {
      throw new Error(
        "二维码在 2 分钟内未出现：主页面 img、#login_container、#login_container iframe 都未就绪",
      );
    }
...
async function renderQrToTerminal(page, qrTarget) {
  if (
    qrTarget?.kind === "login-container" ||
    qrTarget?.kind === "login-iframe" ||
    qrTarget?.kind === "login-frame-img"
  )
    return "";
  try {
